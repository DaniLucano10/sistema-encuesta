...
const ViewForm: React.FC = () => {
  ...
  const [submitted, setSubmitted] = useState<boolean>(() => new URLSearchParams(location.search).get('submitted') === 'true');
  ...
  const handleCheckboxChange = (questionId: string, optionId: string) => {
    setAnswers(prev => {
      ...
      return {
        ...prev,
        [questionId]: currentAnswers.includes(optionId) ? currentAnswers.filter(id => id !== optionId) : [...currentAnswers, optionId],
      };
    });
  };
        ...
        <Box width="90%" maxW="700px" mt={2}>
          ...
          {data.map(item => (
            <Box key={item.id} mb={4} p={4} borderRadius="md" bg="#FFFFFF" borderTop="10px solid #019CFE">
              <Text fontSize="2xl" fontWeight="bold">Detalles de formulario</Text>
              <Text fontSize="xl" fontWeight="bold" color="#2D3748">Cliente: {item.customer_data.name}</Text>
              <Text fontSize="xl" fontWeight="bold" color="#2D2748">Asesor: {item.advisor_data.name}</Text>
            </Box>
          ))}
